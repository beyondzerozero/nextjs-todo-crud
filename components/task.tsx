  ...
  return (
    <>
      <div>
        ...
        <p className="text-xs text-gray-400">최근 변경된 일자: {last_update.toLocaleString("ko-KR")}</p>
      </div>

      <div className="flex">
        <button type="button" className="w-9 text-blue-500 hover:text-blue-600" onClick={() => setShowEditModal(true)}>편집</button>
        <button type="button" className="ml-2 w-9 text-red-500 hover:text-red-600" onClick={() => setShowRemoveModal(true)}>삭제</button>
      </div>
      {showEditModal ? (
